...
public class SampleDocuportStepDefs {
    ...
    public void user_insert_to_field_on_page(String input, String field, String page) {
        switch (page.toLowerCase().trim()) {
            case "login" -> {pages.getLoginPage().insertField(field, input);
                LOG.info(input + " - was successfully sent to - " + field);
            }
            case "received doc" -> {pages.getReceivedDocsPage().insertField(field, input);
                LOG.info(input + " - was successfully sent to - " + field);
            }
    ...
    public void user_clicks_button_on_page(String button, String page) throws InterruptedException {
        switch (page.toLowerCase().trim()) {
            ...
            case "received doc", "received docs" -> {
...
//        element.sendKeys("/Users/guldenyestekova/Desktop/file.txt");

        //BrowserUtils.uploadFileForWindows("/Users/nsh/Desktop/text.txt");
        //BrowserUtils.uploadFileForMac("/Users/nsh/Desktop/text.txt");
        Thread.sleep(3000);
        BrowserUtils.uploadFileUsingAppleScript("/Users/guldenyestekova/Desktop/file.txt");
